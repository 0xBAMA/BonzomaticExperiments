...
void drawLine ( vec3 p0, vec3 p1, vec3 color ) {
  vec2 dims = textureSize( texPreviousFrame, 0 ).xy;
    ...
    uint depthSample = imageLoad( computeTexBack[ 2 ], ivec2( x, y ) ).r;
    float depthSampleRemapped = ( float( depthSample ) / float( 0xFFFFFFFFu ) ) * 0.5f;
		if ( depth < depthSampleRemapped ) {
...
float de ( vec3 p ) {
  ...
  */
  return distance( vec3( 0.0f ), p ) - 0.3f;
}
...
void main ( void ) {
  ...
  uint depthToWrite = uint( ( distSum / ( xIters * yIters ) ) * ( 0xFFFFFFFFu / 2u ) );
  imageStore( computeTex[ 2 ], ivec2( U.xy ), ivec4( depthToWrite ) );
  
  color = vec3( 1.0f - depthToWrite / float( 0xFFFFFFFFu ) );
  ...
  if ( boundsChecks.x && boundsChecks.y || 
    ...
    updateProbe( ivec2( U ) );
         ...
         vec3 direction = normalize( sphericalFibonacci( U.x + 96 * U.y, 96 * 64 * 10 ) );
         direction.zy = rot( 0.5f ) * direction.zy;
         // vec3 direction = normalize( sphericalFibonacci( U.x, 96 ) );
         vec3 origin = vec3( -0.5f * cos( T * 0.33f ), 0.5f * sin( T ), -0.5f );
         float d = rayMarch( origin, direction );
         drawLine( origin, origin + d * direction, vec3( 0.2f * d, 0.5f * d, 0.0f ) );
